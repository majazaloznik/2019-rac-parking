...
* OK, I get England surplus/transport to be 21% not Leibling's 12 %. Additionally if I go directly to the 17/18 file I can confirm that my totals are correct, or rather that the Excel files have an error: the off-street column does not add up to the total they have down (cell DF455 is wrong), probably something to do with Copeland? But the difference is 250 only (thousands), so it doens't affect the 21 % I get. 
* Next the plot.
...
* commit
* had to sort out ssh authentication. still doens't work. or rather works only sometimes!?
...
* Packrat didn't work - because of V8 not working. WHy did i need V8? 
...
* OK, so still no good answer for gettting ssh-agent to work through Rstudio, continue pushing manually.
...
* OK, so this removes the need for `DiagrammeRsvg`, so should allow me to use packrat properly. * ok, let's try initialising packrat again
* ugh, this is weird, it is still listing diagrammeR and Rsvg, i don't know why though? Ach, they were mistekenly loaded in the instructions. fixed
* packrat initialised
* change packrat.opts option: `vcs.ignore.src: TRUE` so that the actual package sources can be ignored by github. 
* this means that if cloned, oackrat will cause it to download any packages required from cran instead of having them in the github repo. which is fine. 
* very annoyingly at the end of initialising packrat you get the message "initialization complete! there were 50 or more warnings.." but then it restarts the R session, which means you cannot see the warnings.. So i don't knwo what it is. ToDo open issue?
...
* Scotland first
* FFuckssake. the plots are suddenly not rendering anymore. No idea what happened there..Alwyas source the update script?
...
* What is the dea with the "label mutliply defined warnings?" they are not mulitply defined. And only some of them cause an issue? But it's only a warning the pdf is fine!? what gives?

* There were weird symbols in front of the pound sign in one of the tables. They went away, but not sure why? 
* Todo: mention this to Ivo, the weird sign could be an artefact on his computer, hopefully packrat will solve this. 
* Scotland done
...
* Update Scotland template wiht Ivo's comments from 30.8.
...
So yeah, it's finicky, but managable. 
...
* Remove packrat and initialise to renv!
* initialisation ok, additionally manually added knitr and markdown as per warnings. lock file written, commit. 
* also add bookdown
...
OK, so I'll now start working through england.R and the report template. The update script will currenlty only use the begining and compiling sections. I will complete the new data section last. SO as I go along this works for the current year and can test it like that. 

* OK, bibliography. Looks like the budget and income/expenditure sources for england are OK, but not the WPL?
* OK, looks like this neds to be first updated in the 00-orig-data-entry
...
* there's a check in the update script to make sure there are enough rows in the data. This is tricky since England has a bunch of different types of rows: 1 gla, 353 LAs, 9 national parks and 1 national total, thats 364. So i'll put the check in.i guess. 
* OK, updating script works so far: creating new rmd and compiling, with working bibliography.
* commit
* careful with the bidget references, aren't they for the next year? Doubleckecked, all good. 
...
* ok, the budgets, i can just add them all up. But they are originally negative for some reasone. I'll fix that in the england.R scritp. 

# Monday 16.9.2019

* don't forget to use dp.text and dp.tables in FunDec for rounding consistenlty!
...
* no, actually i cannot do that, because the total includes also GLA and national parks, anywya, i'll as ivo
* so then i need to import the data in the 01-orig-import file. done
* now i need to merge it with the other england totals

# Tuesday 17.9.2019

* OK, so budgeted transport expenditure totals are now extraceted in the original data import. This required another function as well. 
...
* damn it, the transposing and formatting somehow changes some NAs into "  NA" chars, which is ridiculous and super annoying. i need to make a repext for htis. 
...
* The there is the issue of getting formatting to rows that start out with a multirow, beacuse the text is too long, this doesn't seem to work with kableextrs (repex), so you instead just paste in directly the latex code into the cells rquired. 
...
* The numbers are pulled into the summary text from the summary thable buy row and column indices, so *make sure the size of the table stays the same always*
* finished trend chart. added budgeted surplus line as well, even though it wasn't in the original. 
* OK, now the london /rest of england split. What a horrible table to read, this would be beautiful with a nice stacked bar chart. *ask*
