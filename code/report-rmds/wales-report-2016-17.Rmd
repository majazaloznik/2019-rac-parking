...
`r ifelse(wal.summary$change[4] < -0.05, paste("fallen by", FunDec(-wal.summary$change[4],1), "%"), ifelse(wal.summary$change[1] > 0.05, paste("increased by", FunDec(wal.summary$change[4], 1), "%"),"stayed approximately the same"))` and the surplus now represents 
`r paste(FunDec(wal.summary$V5[5], 1), "%")` of transport costs. 

Parking makes a 
`r ifelse(wal.summary$V5[5] < eng.trans - 0.05 , "smaller", ifelse(wal.summary$V5[5] > eng.trans + 0.05,  "larger","similar"))` contribution to overall transport expenditure in Wales compared with England where it is around `r FunDec(eng.trans, 1)` % of total transport. 
Since `r FunFisc(4)` income has `r ifelse(wal.summary[1,6] > wal.summary[1,2], "risen", "fallen")` by 
`r FunDec(100*wal.summary[1,6]/wal.summary[1,2] - 100, 1)` % and expenditure has `r ifelse(wal.summary[2,6] > wal.summary[2,2], "risen", "fallen")` by `r FunDec(100*wal.summary[2,6]/wal.summary[2,2] - 100, 1)` %. Over the same period the surplus has `r ifelse(wal.summary[3,6] > wal.summary[3,2], "risen", "fallen")` by `r FunDec(100*wal.summary[3,6]/wal.summary[3,2] - 100, 1)` %. Figure \@ref(fig:fig1) gives a longer term overview of the trends in incomes, expenditures and surpluses. 
...
Overall expenditure has `r ifelse( wal.expend.t[2] > 0, "risen", "fallen")` by £`r FunDec(abs(wal.expend.t[2]),1)` million (`r FunDec(abs(wal.expend.t[1]),1)` %) after a `r ifelse( wal.expend.t[3] > 0, paste0("rise of £", FunDec(wal.expend.t[3], 1), " million last year"), paste0("fall of £", FunDec(abs(wal.expend.t[3]),1), "m last year"))`, with `r FunN2W(expend.bin[2])` councils having increased their costs and `r FunN2W(expend.bin[1])` having reduced their costs. The changes from `r FunFisc(1)` are mapped in Figure \@ref(fig:mapexpend).
...
Table \@ref(tab:propexpend) shows the proportion of income taken up by expenditure. Across Wales it was `r FunDec(wal.expend.of.income[23,6],1)` % compared to `r FunDec(wal.expend.of.income[23,5],1)` % in the previous year, suggesting `r ifelse(wal.expend.of.income[23,6] < wal.expend.of.income[23,5], "improving", "declining")` efficiency in operations. 

Cardiff’s expenditure was `r FunDec(wal.expend.of.income[wal.expend.of.income$auth.name == "Cardiff",6],1)` % of its income having `r ifelse(wal.expend.of.income[wal.expend.of.income$auth.name == "Cardiff",6] < wal.expend.of.income[wal.expend.of.income$auth.name == "Cardiff",5], "fallen", "risen")` since last year, while Swansea's has `r ifelse(wal.expend.of.income[wal.expend.of.income$auth.name == "Swansea",6] < wal.expend.of.income[wal.expend.of.income$auth.name == "Swansea",5], "fallen", "risen")` to `r FunDec(wal.expend.of.income[wal.expend.of.income$auth.name == "Swansea",6],1)` %.
