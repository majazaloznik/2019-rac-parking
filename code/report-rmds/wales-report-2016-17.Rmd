...
compared to the previous fiscal year. Total transport expenditure has
...
`r paste(FunDec(wal.summary[5, 6], dp.text), "%")` of net transport epxenditure. 
...
```{r mapincome, fig.cap = paste("Map of change in parking income from ", FunFisc(1), "to",  FunFisc(), " (ref:foo)"), fig.height = 3, fig.pos = "H"}
# map income change
...
```{r mapincome4, fig.cap = paste("Map of average annual change in parking income over the past four years from", FunFisc(4), "to", FunFisc(), "(ref:foo)"), fig.height = 3}
# map income change
...
```{r mapexpend, fig.cap = paste("Map of change in parking expenditure from ", FunFisc(1), "to",  FunFisc(), " (ref:foo)"), fig.height = 3.5}
...
```{r mapexpend4, fig.cap = paste("Map of average annual change in parking expenditure over the past four years from", FunFisc(4), "to", FunFisc(), "(ref:foo)"), fig.height = 3.5}
# map change in expenditure
...
The largest increase of deficit was incurred by `r wal.deficit.change.top1[1,2]`, which increased their deficit by `r FunDec(wal.deficit.change.top1$change[1], dp.text)` %`r if(nrow(wal.deficit.excluded.top) > 0) paste0(" (excluding ", FunMultiText(wal.deficit.excluded.top$auth.name), " with a deficit under £30,000)")`. The biggest decrease was in `r wal.deficit.change.bottom1[1,2]` where the deficit fell by (`r FunDec(abs(wal.deficit.change.bottom1$change[1]), dp.text)` %)`r if(nrow(wal.deficit.excluded.bottom) > 0) paste0(" (excluding ", FunMultiText(wal.deficit.excluded.bottom$auth.name), " with a deficit under £30,000)")`.
