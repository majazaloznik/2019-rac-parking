...
In England as a whole `r FunFirstup(FunN2W(expend.bin["poz"]))` councils increased their parking expenditures over the past year and `r FunN2W(expend.bin["neg"])` decreased them. `r if(!is.na(expend.bin["zero"])) paste("Another", FunN2W(expend.bin["zero"]), "councils' expenditures  stayed the same.")` `r if(!is.na(expend.bin["na"])) paste("There are also", FunN2W(expend.bin["na"]), "councils that did not report expenditure data in this or the previous fiscal year, so we cannot define the change.")`
