...
plot(sco["dpe.status"], main = "", pal = viridis(3),
...
`r ifelse(sco.summary$change[4] < -0.05, paste("fallen by", FunDec(-sco.summary$change[4], 1), "%"), ifelse(sco.summary$change[1] > 0.05, paste("increased by", FunDec(sco.summary$change[4], 1), "%"),"stayed approximately the same"))` and the surplus now represents `r paste(FunDec(sco.summary$V5[5],1), "%")` of transport costs. Parking makes a 
`r ifelse(sco.summary$V5[5] < eng.trans - 0.05 , "smaller", ifelse(sco.summary$V5[5] > eng.trans + 0.05,  "larger","similar"))` contribution to overall transport expenditure in Scotland compared with England where it is around `r FunDec(eng.trans,1)` % of total transport.
