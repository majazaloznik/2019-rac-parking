...
compared to the previous fiscal year. Total transport costs have
...
Scottish councils with DPE earn `r FunDec(sco.pcn.prop[nrow(sco.pcn.numbers), ncol(sco.pcn.prop)], dp.text)` % of their income from PCNs, which is \color{red}about half of the equivalent proportions for London and for the rest of England \color{black} (see Table \@ref(tab:pcnprop)). 
...
`r sco.deficit.change.top3[1,2]` increased their deficits by `r FunDec(sco.deficit.change.top3$change[1], dp.text)` %
while `r sco.deficit.change.top3[2,2]` and `r sco.deficit.change.top3[3,2]` increased  by `r FunDec(sco.deficit.change.top3$change[2], dp.text)` % and `r FunDec(sco.deficit.change.top3$change[3], dp.text)` % respectively`r if(nrow(sco.deficit.excluded.top) > 0) paste0(" (excluding ", FunMultiText(sco.deficit.excluded.top$auth.name), " with a deficit under £30,000)")`. The biggest decreases were in `r sco.deficit.change.bottom2[2,2]` (`r FunDec(abs(sco.deficit.change.bottom2$change[2]), dp.text)` %), and `r sco.deficit.change.bottom2[1,2]` (`r FunDec(abs(sco.deficit.change.bottom2$change[1]), dp.text)`  %)`r if(nrow(sco.deficit.excluded.bottom) > 0) paste0(" (excluding ", FunMultiText(sco.deficit.excluded.bottom$auth.name), " with a deficit under £30,000)")`.
