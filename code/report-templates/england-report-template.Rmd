...
`r FunFirstup(FunN2W(income.bin[2]))` councils increased their income over the past year and `r FunN2W(income.bin[1])` decreased their income. `r if (eng.no.income > 0 ) {paste0(" ", FunFirstup(FunN2W(eng.no.income)), " councils did not show any income in ", FunFisc(), ".")}` 

The top three English counils by income were `r paste0(names(eng.income.top3)[1], ", ", names(eng.income.top3)[2], ", and ", names(eng.income.top3)[3])`, and between them accounted for `r FunDec(eng.income.top3[1], dp.text)` % of parking income. 
