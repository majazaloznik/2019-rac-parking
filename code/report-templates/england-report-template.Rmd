...
Table \@ref(tab:compare) provides a comparison with London, England excluding London, Wales, and Scotland for the most recent available data, while Table \@ref(tab:change) compares the changes between `r FunFisc()` and the previous year, with the average annual change over the four-year period starting in `r FunFisc(4)` (or the most recent four-year period for which data is available). In the last year the surpluses for London have increased by `r FunDec(lond.ann, dp.text)` %, which is `r ifelse(lond.ann < lond.4av, "less", "more")` than the average annual increase observed over the preceding four years, which was `r FunDec(lond.4av, dp.text)` %. For the rest of England excluding London the surpluses have increased by `r FunDec(rest.ann, dp.text)` %, which is `r ifelse(rest.ann < rest.4av, "less", "more")` than the four-year average annual increase at `r FunDec(rest.4av, dp.text)` %.
...
Parking income is derived from three main sources: meter income, residents’  business permits, and penalties. Other sources are towing and storage charges although relatively few councils operate pounds now. Total income in `r FunFisc()` was £`r formatC(inc.tot, big.mark = ",")` million, `r FunDec(inc.ch, dp.text)`% higher than in `r FunFisc(1)`. In London boroughs the total income was £`r FunDec(inc.london.tot, dp.text)`million, `r FunDec(inc.london.tot.ch, dp.text)` % more than in the previous year, while the total for the rest of England was £`r FunDec(inc.rest.tot, dp.text)`million, `r FunDec(inc.rest.tot.ch, dp.text)` % more than in the previous year. 

Income from on-street tickets and permits was £`r FunDec(inc.fnp, 0)` million, (`r ifelse(inc.fnp.ch>0, "up", "down")` `r FunDec(inc.fnp.ch, dp.text)` %), and a further £`r FunDec(inc.pcn, 0)` million came from on-street penalty charges (`r ifelse(inc.pcn.ch>0, "up", "down")` `r FunDec(inc.pcn.ch, dp.text)` %) and £`r FunDec(inc.off, 0)` million (`r ifelse(inc.off.ch>0, "up", "down")` `r FunDec(inc.off.ch, dp.text)` %)from off-street charges and penalties. Note that this does not include off-street income received by commercial off-street parking facilities. Table \@ref(tab:incometablondon) shows the incomes for all the London boroughs for the previous 5 years, their most recent annual change and the average annual change in income over the preceding 5 years. Table \@ref(tab:incometabrest) has the same information for the top 20 councils outside of London. For a full table of all the local authrities in London see Table \@ref(tab:incometab)  in the Appendix. 
...
eng.income.london.formatted %>% 
  kable( "latex", 
         ...
         longtable = T,
         align = c("r"),
         caption = "Parking income for London boroughs (£,000)",
         format.args = list(big.mark = ","),
         linesep = "",
...
In England as a whole `r FunFirstup(FunN2W(income.bin[2]))` councils increased their income over the past year and `r FunN2W(income.bin[1])` decreased their income. `r if (eng.no.income > 0 ) {paste0(" ", FunFirstup(FunN2W(eng.no.income)), " councils did not show any income in ", FunFisc(), ".")}` 
...
eng.income.rest.formatted %>% 
  ...
  kable( "latex", 
         ...
         longtable = T,
         align = c("r"),
         caption = "Parking income for the rest of England outside London (£,000)",
         format.args = list(big.mark = ","),
         linesep = "",
         col.names = c("\\multirow{1}{*}[0pt]{Local Authority}", yearz, 
                       paste0("Change ", FunFisc(), 
  ...
  column_spec(7, width = "1.5cm") %>% 
  column_spec(8, width = "1.5cm") %>% 
  row_spec(c(21, 23), hline_after = TRUE)
...
Overall expenditure on parking rose to £`r FunDec(exp.tot,0)`million, up `r FunDec(exp.ch , dp.text)`% from `r FunFisc(1)` for both on-street and off-street. Expenditure in London went `r ifelse( exp.london.tot.ch > 0, "up", "down")` by `r FunDec(exp.london.tot.ch, dp.text)` % and `r ifelse(sign(exp.london.tot.ch == exp.rest.tot.ch), "", ifelse(exp.rest.tot.ch > 0, "up", "down"))` by `r FunDec(exp.rest.tot.ch, dp.text)` % in the rest of England. Expenditure represents `r FunDec(exp.prop.inc.on, dp.text)` % of income for on-street parking and `r FunDec(exp.prop.inc.off, dp.text)` % for off-street. 

Table \@ref(tab:expendtablondon) ranks the London boroughs in terms of expenditure on parking while thable  \@ref(tab:expendtabrest) lists the top 20 English councils outside London. See Table \@ref(tab:expendtab) in the Appendix for a comprehensive list of all 353 local authorities in England. 


Overall expenditure has `r ifelse( eng.expend.t[2] > 0, "risen", "fallen")` by £`r FunDec(abs(eng.expend.t[2]), dp.text)` million (`r FunDec(abs(eng.expend.t[1]), dp.text)` %) after a `r ifelse( eng.expend.t[3] > 0, paste0("rise of £", FunDec(eng.expend.t[3], dp.text), " million last year"), paste0("fall of £", FunDec(abs(eng.expend.t[3]), dp.text), " million last year"))`, with `r FunN2W(expend.bin[2])` councils having increased their costs and `r FunN2W(expend.bin[1])` having reduced them. 
...
In the final columns Tables \@ref(tab:expendtablondon) and  \@ref(tab:expendtabrest) also show the proportion of income taken up by costs in each authority for `r FunFisc()`. For London boroughs as a group `r FunDec(prop.income.summary[1], dp.text)` % of their parking income was used for expenditure, while for the other authorities in England was  `r FunDec(prop.income.summary[2], dp.text)` %, and for England as a whole the number was `r FunDec(prop.income.summary[3], dp.text)` %. 
...
eng.expend.london.formatted %>% 
  kable( "latex", 
         ...
         longtable = TRUE,
         align = c("r"),
         caption = "Parking expenditure for London boroughs (£,000)",
         format.args = list(big.mark = ","),
         linesep = "",
         col.names = c("\\multirow{1}{*}[0pt]{Local Authority}", yearz, 
                       paste0("Change ", FunFisc(), 
...
The total *surpluses* for London amounted to £`r FunDec((london.surplus.totals.table.tot), dp.text)` million between `r  london.poz.surplus ` authorities, of which the top three--`r paste0(names(london.surplus.top3)[1], ", ", names(london.surplus.top3)[2], ", and ", names(london.surplus.top3)[3])`--accounted for `r FunDec(london.surplus.top3[1],  dp.text)` %.  `r FunFirstup(FunN2W(surplus.bin["neg"]))` `r ifelse(surplus.bin["neg"] == 1, "council", "councils")` made a loss with the total of parking deficits `r ifelse(london.surplus.totals.table.def > london.surplus.totals.table.def.last, "falling", "rising")` to £`r FunDec(-london.surplus.totals.table.def, dp.text)` million from £`r FunDec(-london.surplus.totals.table.def.last, dp.text)` million last year. 

Overall, parking contributed £`r FunDec(london.surplus.totals.table.tot, dp.text)` million to local authority finances in London in `r FunFisc()` compared with £`r FunDec(london.surplus.totals.table.tot.last, dp.text)` million in `r FunFisc()`, an increase of `r FunDec(london.surplus.totals.table.ch, dp.text)` %. 
...
rest.surplus.totals.table.full.formatted %>% 
  kable("latex", 
         ...
         escape = F,
         align = c("r"),
         caption = "Parking surpluses (on-street and off-street) for all councils outside London (£ millions)",
         linesep = "",
         col.names = c("\\multirow{1}{*}[0pt]{Local Authority}", yearz, 
                       paste0("Change ", current.year, "-", current.year-1999, 
                              " on ", current.year-1, "-", current.year-1999-1))) %>% 
