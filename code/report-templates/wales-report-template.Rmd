...
Table @ref(tab:compare) provides a comparison with London, England excluding London, and Wales for the most recent available data. 

\renewcommand{\arraystretch}{1.2}
```{r compare, echo = FALSE}
# comparing summaries for all three countries
# extract relevant subset of data for england
master %>% 
  filter(country == "England") %>% 
  ...
  mutate(income.total = income.on + income.off,
         expend.total = expend.on + expend.off,
         surplus.total = income.total - expend.total,
         country = recode(auth.type, "X" = "X", "L" = "London",
                        .default = "England without London")) %>% 
  group_by(country) %>% 
  filter(country != "X") %>% 
  summarise( sum.year = first(year),
            income = sum(income.total, na.rm = TRUE),
            expend = sum(expend.total, na.rm = TRUE),
             surplus = sum(surplus.total, na.rm = TRUE),
            prop.of.income = surplus/income) -> eng.sum

# extract relevant subset of data for wales
master %>% 
  filter(country == "Wales") %>% 
  filter(year == min(current.year, max(year))) %>% 
  ...
  summarise( sum.year = first(year),
            ...
            expend = sum(expend.total, na.rm = TRUE),
            surplus = sum(surplus.total, na.rm = TRUE),
            prop.of.income = surplus/income) -> wal.sum

# extract relevant subset of data for Scotland
master %>% 
  ...
  summarise( sum.year = first(year),
            ...
            expend = sum(expend.total, na.rm = TRUE),
            surplus = sum(surplus.total, na.rm = TRUE),
            prop.of.income = surplus/income) -> sco.sum

# bind all together and reshape
bind_rows(eng.sum, wal.sum, sco.sum) %>% 
  mutate(sum.year = paste0("(", sum.year, "-", sum.year-1999, ")"),
  ...
  mutate(country = c("Fiscal year", "Parking income", "Parking expenditure", "Surplus", "Surplus as proportion if income")) -> sum.gb
...
sum.gb %>% 
   kable( "latex", 
         ...
         longtable = T,
         align = c("r"),
         col.names = c("", "England without London", "London", "Wales", "Scotland"),
         caption = paste0("Comparison of parking income and expenditure in across the nations of Great Britain"), 
...
Overall expenditure has `r ifelse( wal.expend.t[2] > 0, "risen", "fallen")` by £`r FunDec(abs(wal.expend.t[2]),1)`m (`r FunDec(abs(wal.expend.t[1]),1)` %) after a `r ifelse( wal.expend.t[3] > 0, paste0("rise of £", FunDec(wal.expend.t[3], 1), "m last year"), paste0("fall of £", FunDec(abs(wal.expend.t[3]),1), "m last year"))`, with `r FunN2W(expend.bin[2])` councils having increased their and `r FunN2W(expend.bin[1])` reduced their costs.  
...
Table @ref(tab:expendprop) shows the proportion of income taken up by expenditure. Across Wales it was `r FunDec(wal.expend.of.income[23,6],1)` % having `r ifelse(wal.expend.of.income[23,6] < wal.expend.of.income[23,5], "fallen", "risen")` compared to `r FunDec(wal.expend.of.income[23,5],1)` % in the previous year, suggesting `r ifelse(wal.expend.of.income[23,6] < wal.expend.of.income[33,5], "improving", "declining")` efficiency in operations. 
...
bind_rows(wal.surplus, wal.surplus.totals) %>% 
  mutate(change =100*(.[[6]]/.[[5]]-1),
         prop.transp = 100*.[[6]]/.[[7]]) %>% 
  mutate(change = ifelse(sign(!!as.name(current.year)) != 
                           sign(!!as.name(current.year-1)), NA, change)) %>% 
...
while `r wal.surplus.change.top3[2,2]` and `r wal.surplus.change.top3[3,2]` increased  by `r FunDec(wal.surplus.change.top3$change[2], 0)` % and `r FunDec(wal.surplus.change.top3$change[3], 0)` % respectively`r if(nrow(wal.surplus.excluded.top) > 0) paste0(" (excluding ", FunMultiText(wal.surplus.excluded.top$auth.name), " due to very low numbers)")`. The biggest decreases were in `r wal.surplus.change.bottom2[2,2]` (`r FunDec(abs(wal.surplus.change.bottom2$change[2]), 0)` %), and `r wal.surplus.change.bottom2[1,2]` (`r FunDec(abs(wal.surplus.change.bottom2$change[1]), 0)`  %)`r if(nrow(wal.surplus.excluded.bottom) > 0) paste0(" (excluding ", FunMultiText(wal.surplus.excluded.bottom$auth.name), " due to very low numbers)")`.
