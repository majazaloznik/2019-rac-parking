...
`r FunFirstup(xfun::numbers_to_words(surplus.bin[2]))` councils showed surpluses and `r xfun::numbers_to_words(surplus.bin[1])` showed deficits \color{red}(the same three councils as 2016-17). \color{black} All councils are now receiving income from parking, \color{red} although councils such as Blaenau Gwent (Ebbw Vale and Abertillery) and Torfaen (Pontypool and Cwmbran) offer free parking but still issue penalty charge notices. \color{black}
...
wal.income %>% 
  arrange(desc(change)) %>% 
  filter(row_number() %in% c(23, 22) &
           .[[6]] <= 30) -> wal.income.exclude

wal.income %>% 
  arrange(desc(change)) %>% 
  filter(row_number() %in% c(1, 2, 23-nrow(wal.income.exclude), 
                             22-nrow(wal.income.exclude))) %>% 
...
while `r top.bottom[2,1]` increased its income by `r format(round(top.bottom[2,2], 0), nsmall = 0)` %. The largest falls were in `r top.bottom[4,1]` (`r format(round(top.bottom[4,2], 0), nsmall = 0)` %), and `r top.bottom[3,1]` (`r format(round(top.bottom[3,2], 0), nsmall = 0)` %)`r ifelse(nrow(wal.income.exclude) == 0, ".", ifelse(nrow(wal.income.exclude) == 1, paste(", excluding", wal.income.exclude[1], "where the income is very low."), paste(", excluding", wal.income.exclude[1], "and", wal.income.exclude[2], "where the income is very low.")))`
...
wal.expend %>% 
  filter(auth.name == "Total") %>% 
  mutate(current.change = format(round((.[[6]]-.[[5]])/1000,1), nsmall = 1),
         last.change = format(round((.[[5]]-.[[4]])/1000,1), nsmall = 1),
         change = format(round(change,1),nsmall = 1)) %>% 
...
wal.expend %>% 
  filter(auth.name != "Total") %>% 
  arrange(desc(change)) %>% 
  filter(row_number() %in% c(1, 2, 3, 21, 22)) %>% 
  select(auth.name, change, 6) -> top.bottom
...
Overall expenditure has risen by £`r wal.expend.t[2]`m (`r wal.expend.t[1]` %) after a `r ifelse( wal.expend.t[3] > 0, paste0("rise of £", wal.expend.t[3], "m last year"), paste0("fall of £", wal.expend.t[3], "m last year"))`, with `r xfun::numbers_to_words(expend.bin[2])` councils having increased their and `r xfun::numbers_to_words(expend.bin[1])` reduced their costs.  

`r top.bottom[1,1]` increased expenditure by `r format(round(top.bottom[1,2], 0), nsmall = 0)` %
while `r top.bottom[2,1]` and `r top.bottom[3,1]` increased  by `r format(round(top.bottom[2,2], 0), nsmall = 0)` % and `r format(round(top.bottom[3,2], 0), nsmall = 0)` % respectively. The biggest decreases were in `r top.bottom[5,1]` (`r format(round(top.bottom[5,2], 0), nsmall = 0)` %), and `r top.bottom[4,1]` (`r format(round(top.bottom[4,2], 0), nsmall = 0)` %). \color{red} Newport’s expenditure is likely to rise when they take over parking
management from Gwent Police.\color{black}

The table also shows the proportion of income taken up by costs in 2017-18.
Nationally in Wales it is `r format(round(wal.expend.p[2],1), nsmall = 1)` % with Cardiff at `r format(round(wal.expend.p[1],1), nsmall = 1)` .
...
Total parking surpluses amount to £`r format(round((wal.surplus.totals.table[24, 6]/1000),1), nsmall = 1)`m between `r  sum(wal.surplus.t3$count) ` authorities of which Cardiff,
Swansea and Gwynedd contribute `r  format(round(100*wal.surplus.t3$prop[2],1), nsmall = 1)`%. \color{red} Newport increased its surplus significantly because of the sharp reduction in cost. \color{black} `r FunFirstup(xfun::numbers_to_words(surplus.bin[1]))` councils made a loss with the total of parking deficits 
`r ifelse(wal.surplus.totals.table[23,6] > wal.surplus.totals.table[23,5], "falling", "rising")` to £`r format(round(-wal.surplus.totals.table[23,6]/1000,1), nsmall = 1)`m from `r format(round(-wal.surplus.totals.table[23,5]/1000,1), nsmall = 1)` last year \color{red} mainly because of a reduction
in the loss in Blaenau Gwent with its big fall in expenditure.\color{black}
