...
Table @ref(tab:sumtab) and Figure X@ref(fig:fig1) show the summary accounts for local authorities in Scotland for fiscal years `r paste0(current.year-4, "-", current.year-1999-4)` to `r paste0(current.year, "-", current.year-1999)`. The income has 
...
sco.income %>% 
  ...
  group_by(change) %>% 
  summarise(n = n()) %>% 
  tibble::deframe() -> income.bin

# select only valid
sco.income %>% 
  filter(auth.name != "Total") %>% 
  arrange(desc(change)) %>% 
  filter(!is.nan(change) & !is.infinite(change)) -> sco.income.valid
...
# extract bottom LAs where income is under 30.000
 sco.income.valid %>% 
  filter(row_number() %in% c(n(), n()-1) & !!as.name(current.year) <= 30) -> sco.income.exclude.bottom

# extract top LAs where expenditure is under 30.000
 sco.income.valid %>% 
  filter(row_number() %in% c(1, 2, 3) & !!as.name(current.year) <= 30) -> sco.income.exclude.top

# remove excluded rows and select top 3 and bottom 2
sco.income.valid %>% 
  anti_join(sco.income.exclude.top) %>% 
  anti_join(sco.income.exclude.bottom) %>% 
  filter(row_number() %in% c(1, 2, 3, n()-1, n())) %>% 
  select(auth.name, change, !!as.name(current.year)) -> sco.income.top.bottom

```

`r FunFirstup(xfun::numbers_to_words(income.bin[2]))` councils increased their income over the past year and `r xfun::numbers_to_words(income.bin[1])` decreased their income. `r if (sco.no.income > 0 ) {paste0(FunFirstup(xfun::numbers_to_words(sco.no.income)), " councils did not show any income in ", FunFisc(), ".")}`
...
`r sco.income.top.bottom[1,1]` increased their income by `r FunDec(sco.income.top.bottom[1,2], 0)` %
while `r sco.income.top.bottom[2,1]` and `r sco.income.top.bottom[3,1]` increased  by `r FunDec(sco.income.top.bottom[2,2], 0)` % and `r FunDec(sco.income.top.bottom[3,2], 0)` % respectively`r ifelse(nrow(sco.income.exclude.top) == 0, ".", ifelse(nrow(sco.income.exclude.top) == 1, paste(", excluding", sco.income.exclude.top[1], "where the income is very low."), ifelse(nrow(sco.income.exclude.top) == 2,  paste(", excluding", sco.income.exclude.top[1,1], ", and", sco.income.exclude.top[2,1], "where the income is very low."), paste0(", excluding ", sco.income.exclude.top[1,1], ", ", sco.income.exclude.top[2,1], ", and ", sco.income.exclude.top[3,1], " where the income is very low."))))` The biggest decreases were in `r sco.income.top.bottom[5,1]` (`r FunDec(abs(sco.income.top.bottom[5,2]), 0)` %), and `r sco.income.top.bottom[4,1]` (`r FunDec(abs(sco.income.top.bottom[4,2]),0)` %) `r ifelse(nrow(sco.income.exclude.bottom) == 0, ".", ifelse(nrow(sco.income.exclude.bottom) == 1, paste(" (excluding", sco.income.exclude.bottom[1], "where the income is very low)."),   paste(" (excluding", sco.income.exclude.bottom[1,1], ", and", sco.income.exclude.bottom[2,1], "where the income is very low).")))`
...
sco.expend %>% 
  filter(auth.name != "Total") %>% 
  arrange(desc(change)) %>% 
  filter(!is.nan(change) & !is.infinite(change)) -> sco.expend.valid
    
# extract bottom LAs where expenditure is under 30.000
 sco.expend.valid %>% 
  filter(row_number() %in% c(n(), n()-1) & !!as.name(current.year) <= 30) -> sco.expend.exclude.bottom

# extract top LAs where expenditure is under 30.000
 sco.expend.valid %>% 
  filter(row_number() %in% c(1, 2, 3) & !!as.name(current.year) <= 30) -> sco.expend.exclude.top

# remove excluded rows and select top 3 and bottom 2
sco.expend.valid %>% 
  anti_join(sco.expend.exclude.top) %>% 
  anti_join(sco.expend.exclude.bottom) %>% 
  filter(row_number() %in% c(1, 2, 3, n()-1, n())) %>% 
  select(auth.name, change, !!as.name(current.year)) -> sco.expend.top.bottom
...
The largest increase in expenditure happened in`r sco.expend.top.bottom[1,1]`, where it increased  by `r FunDec(sco.expend.top.bottom[1,2], 0)` %, 
while `r sco.expend.top.bottom[2,1]` and `r sco.expend.top.bottom[3,1]` increased  by `r FunDec(sco.expend.top.bottom[2,2], 0)` % and `r FunDec(sco.expend.top.bottom[3,2], 0)` % respectively 
`r ifelse(nrow(sco.expend.exclude.top) == 0, ".", ifelse(nrow(sco.expend.exclude.top) == 1, paste(", excluding", sco.expend.exclude.top[1], "where the expenditure spending is very low."), ifelse(nrow(sco.expend.exclude.top) == 2,  paste(", excluding", sco.expend.exclude.top[1,1], ", and", sco.expend.exclude.top[2,1], "where the expenditure spending is very low."), paste0(", excluding ", sco.expend.exclude.top[1,1], ", ", sco.expend.exclude.top[2,1], ", and ", sco.expend.exclude.top[3,1], " where the expenditure spending is very low."))))` The biggest decreases were in `r sco.expend.top.bottom[5,1]` (`r FunDec(abs(sco.expend.top.bottom[5,2]), 0)` %), and `r sco.expend.top.bottom[4,1]` (`r FunDec(abs(sco.expend.top.bottom[4,2]),0)` %) `r ifelse(nrow(sco.expend.exclude.bottom) == 0, ".", ifelse(nrow(sco.expend.exclude.bottom) == 1, paste(" (excluding", sco.expend.exclude.bottom[1], "where the expenditure spending is very low)."),   paste(" (excluding", sco.expend.exclude.bottom[1,1], ", and", sco.expend.exclude.bottom[2,1], "where the expenditure spending is very low).")))`
